...
function initSseNotifications() {
    ...
    evtSource.addEventListener('newBoard', (e) => {
        let payload;
        try {
        ...
        showToast(`Użytkownik ${payload.creator_username} utworzył nową planszę: "${payload.board_name}".`, () => { window.location.href = `/play/${payload.board_id}/`; });
    });
    // newPath
    evtSource.addEventListener('newPath', (e) => {
        let payload;
        try {
            payload = JSON.parse(e.data);
        }
        ...
        showToast(`Użytkownik ${payload.user_username} zapisał ścieżkę na planszy: "${payload.board_name}".`);
    });
}
// Automatyczne wywołanie przy załadowaniu strony
