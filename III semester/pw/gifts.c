...
int main(void)
{
    ...
    shared_storage->n_gifts_to_deliver = N_ELFS * LOOPS;
    sem_t* mutex = (sem_t*)&shared_storage->mutex;
    ASSERT_SYS_OK(sem_init(mutex, 1, 1));
    sem_t* sem_free_slots = (sem_t*)&shared_storage->sem_free_slots;
    ASSERT_SYS_OK(sem_init(sem_free_slots, 1, BUFFSIZE));
    sem_t* sem_filled_slots = (sem_t*)&shared_storage->sem_filled_slots;
    ASSERT_SYS_OK(sem_init(sem_filled_slots, 1, 0));
    ...
    ASSERT_SYS_OK(sem_destroy(sem_filled_slots));
    ASSERT_SYS_OK(sem_destroy(sem_free_slots));
